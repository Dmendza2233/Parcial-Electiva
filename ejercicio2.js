...
function agregarLibro() {
    entrada.question("titulo del libro: ", (titulo) => {
        if (!validoT(titulo))
            return error(agregarLibro, "titulo no puede estar vacio");

        entrada.question("autor: ", (autor) => {
            if (!ValidoT(autor))
                return error(agregarLibro, "autor no puede estar vacio");

            entrada.question("año de publicación: ", (año) => {
                if (!validoA(año))
                    return error(agregarLibro, "El año debe ser un numero positivo");

                entrada.question("ISBN: ", (isbn) => {
                    if (!validoT(isbn))
                        return error(agregarLibro, "ISBN no puede estar vacio");
...
function listarLibros() {
    if (biblioteca.length === 0) {
        ...
        biblioteca.forEach((libro, i) => {
            console.log(
                `${i + 1}. "${libro.titulo}" - ${libro.autor} (${libro.anio}) [ISBN: ${libro.isbn
                }]`
            );
        });
    }
...
function buscarLibros() {
    entrada.question("ingresa titulo o autor a buscar: ", (criterio) => {
        ...
        const resultados = biblioteca.filter(
            (libro) =>
